...
public class RedisClusterMessageBus implements IClusterMessageBus {
    ...
    String serialize(ClusterMessage msg) throws Exception {
        IMCommand cmd = msg.getCommand();
        Map<String, Object> cmdMap = cmd.toJsonMap();

        byte[] body = cmd.getBody();
        if (body != null && body.length > 0) {
            cmdMap.put("_body", Base64.getEncoder().encodeToString(body));
        }
    ...
    ClusterMessage deserialize(String json) throws Exception {
        ...
        Map<String, Object> cmdMap = (Map<String, Object>) root.get("command");
        IMCommand cmd = IMCommand.fromJsonMap(cmdMap);

        String bodyB64 = (String) cmdMap.get("_body");
        if (bodyB64 != null && !bodyB64.isEmpty()) {
            cmd.setBody(Base64.getDecoder().decode(bodyB64));
        }

        ClusterMessage.Kind kind = ClusterMessage.Kind.valueOf(kindStr);
        return new ClusterMessage(kind, fromNodeId, cmd, ttl);
    }
