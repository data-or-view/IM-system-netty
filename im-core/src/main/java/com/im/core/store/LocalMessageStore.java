 ...
 *
 * 双索引存储：
 *   · Conversation 存储：conversationId → List(Message)（按 seq 有序，基于 _ms 头）
 *   · 离线消息队列：userId → Queue(Message)（FIFO，上限 1000 条）
 *
 * 保存时同时写入两个索引：
...
public class LocalMessageStore implements IMessageStore {
    ...
    private final ConcurrentMap<String, CopyOnWriteArrayList<Message>> conversationStore = new ConcurrentHashMap<>();

    /** userId → 离线消息队列（FIFO） */
    private final ConcurrentMap<String, ConcurrentLinkedQueue<Message>> offlineStore = new ConcurrentHashMap<>();
    ...
    public List<Message> pullBySequence(String conversationId, long startSeq, long endSeq, int limit) {
        ...
        List<Message> result = new ArrayList<>(actualLimit);
    ...
    public List<Message> pullOffline(String userId, int limit) {
        ConcurrentLinkedQueue<Message> queue = offlineStore.get(userId);
        if (queue == null || queue.isEmpty()) {
            return Collections.emptyList();
        }

        List<Message> result = new ArrayList<>(Math.min(limit, queue.size()));
        for (int i = 0; i < limit && !queue.isEmpty(); i++) {
            Message msg = queue.poll();
            log.debug("Pulling offline msg for user {}: messageId={}, from={}",
                    userId, msg.getMessageId(), msg.getFromUserId());
            result.add(msg);
        }
        return result;
    }
    ...
    /**
     * 从 Message 中提取 _ms 头作为 long。
     */
    private long seqOf(Message msg) {
        return msg.getMessageSeq();
    }
     ...
     * 群聊：group_{groupId}
     */
    private String conversationId(Message msg) {
        String groupId = msg.getGroupId();
        if (groupId != null) {
