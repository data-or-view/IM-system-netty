 ...
 *   ① 验证 userId（后续接入密码/验证码验证）
 *   ② 签发 token（IAuthenticator.issueToken）
 *   ③ sessionManager.bindUser → 绑定 userId 到 channel，踢旧
 *   ④ routeTable.online → 注册用户到集群路由表
...
public class LoginHandler implements IMessageHandler {
    ...
    public void handle(ChannelHandlerContext ctx, IMCommand msg) {
        ...
        // ② 绑定 userId 到当前 channel（踢旧连接）
        sessionManager.bindUser(ctx.channel(), userId);

        // ③ 注册到路由表（通知集群：该用户在本节点在线）
        if (routeTable != null) {
            routeTable.online(userId, localNodeId);
            log.info("Route registered: userId={}, node={}", userId, localNodeId);
        }
        ...
        log.info("User logged in: userId={}, remote={}", userId, ctx.channel().remoteAddress());
    }
