...
export class UserAPI {
  ...
  register(params: { password?: string; nickname?: string; faceUrl?: string } = {}): Promise<RegisterResult> {
    return requireHttp(this.httpTransport).post<RegisterResult>("/api/user/register", {
      ...(params.password ? { password: params.password } : {}),
      ...(params.nickname ? { nickname: params.nickname } : {}),
      ...(params.faceUrl ? { faceUrl: params.faceUrl } : {}),
    });
  }

  /** @deprecated userId is generated by the server. Use register({ password, nickname, faceUrl }) instead. */
  registerLegacy(_userId: string, password?: string, nickname?: string, faceUrl?: string): Promise<WSResponse> {
